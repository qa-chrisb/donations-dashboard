...
export async function loadDonations() {
  ...
  const results = await Promise.all(
    providers.map((p) => p.fetchSince(sinceISO())),
  );
  ...
  for (const d of donations) {
    ...
    daily_by_currency[d.currency][day] =
      (daily_by_currency[d.currency][day] ?? 0) + val;
    // provider share
    provider_by_currency[d.currency] ??= {};
    provider_by_currency[d.currency][d.provider] =
      (provider_by_currency[d.currency][d.provider] ?? 0) + val;
  }
